...
class financeiro {
  ...
  adicionaNovoFinanceiro(req, res) {
    ...
    VALUES ('${req.body.id_venda_fk}', ${parseInt(
      req.body.ValorVenda
    )}, ${parseInt(req.body.ValorPedido)}, ${req.body.ValorLucro})`;
  ...
  selectLista(res, id) {
    let sql =
      "select p.valor, p.produtos from venda v " +
      "INNER JOIN pedido p " +
  ...
  editarFinanceiro(id, financeiro, res) {
    let sql = "UPDATE financeiro SET ?" + `WHERE id_financeiro_pk = ${id}`;
